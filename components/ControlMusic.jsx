...
export default function ControlMusic() {
  ...
  useEffect(() => {
    ...
    window.electronAPI.ReceiveFromElectron("playable-music", handler);
  }, []);
  ...
  function handleNext() {
    if (musicPlaylist.length - 1 > musicIndex) {
      const nextIndex = musicIndex + 1;
      setMusicIndex(nextIndex);
      setAudio(`/musics/${musicPlaylist[nextIndex]}`);
      document.getElementById("play").classList.add("hidden");
      document.getElementById("pause").classList.remove("hidden");
    }
  }
  ...
  function handlePrevious() {
    if (musicIndex > 0) {
      ...
      setAudio(`/musics/${musicPlaylist[prevIndex]}`);
      document.getElementById("play").classList.add("hidden");
      document.getElementById("pause").classList.remove("hidden");
    }
  }
